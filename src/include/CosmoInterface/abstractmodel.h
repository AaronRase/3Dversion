...
#define MakeModel(_ModelName, _ModelParsType) AbstractModel<MakeAbstractModelTemplateArgs(_ModelName, _ModelParsType, double)>
    ...
    public:
        ...
        std::unique_ptr<FieldCollection<Field, T, 6, true>> ETC; //our own contribution
        ...
        AbstractModel(ParameterParser& parser, const LatticeParameters<T>& par, std::shared_ptr<MemoryToolBox> toolBox, T pDt, std::string pName="") :
          ...
          ETC( parser.get<bool>("withETC", false) ? std::make_unique<FieldCollection<Field, T, 6, true>>("ETC", toolBox, par) : nullptr) //our own contribution
          {
            ...
            if(NDim != 3 && ETC != nullptr) throw(RunParametersInconsistent("NDims must be equal to 3 to run ETC. If you want to run with NDim != 3, make sure withETC = false."));
        ...
        auto ETCtensor(Tag<1>, Tag<1>) {return (*ETC)(0_c);} //our own contributions
        auto ETCtensor(Tag<1>, Tag<2>) {return (*ETC)(1_c);}
        auto ETCtensor(Tag<1>, Tag<3>) {return (*ETC)(2_c);}
        auto ETCtensor(Tag<2>, Tag<1>) {return (*ETC)(1_c);}
        auto ETCtensor(Tag<2>, Tag<2>) {return (*ETC)(3_c);}
        auto ETCtensor(Tag<2>, Tag<3>) {return (*ETC)(4_c);}
        auto ETCtensor(Tag<3>, Tag<1>) {return (*ETC)(2_c);}
        auto ETCtensor(Tag<3>, Tag<2>) {return (*ETC)(4_c);}
        auto ETCtensor(Tag<3>, Tag<3>) {return (*ETC)(5_c);}
