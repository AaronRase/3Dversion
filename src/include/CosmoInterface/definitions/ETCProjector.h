#ifndef COSMOINTERFACE_HELPERS_ETCPROJECTOR_H
#define COSMOINTERFACE_HELPERS_ETCPROJECTOR_H
...
namespace TempLat {
     ...
     *
     *
     * Unit test: make test-ETCProjector
     **/

    class ETCProjector {
    public:
        /* Put public methods here. These should change very little over time. */
        ETCProjector(int PRJType):
        mType(PRJType)
        {
        }
...
//         }
        ...
        auto projectedETC_complex(Model& model, Looper& it) {
            ...
            auto u11 = GetValue::get(model.ETCtensor(1_c,1_c).inFourierSpace(), it());
            auto u12 = GetValue::get(model.ETCtensor(1_c,2_c).inFourierSpace(), it());
            auto u13 = GetValue::get(model.ETCtensor(1_c,3_c).inFourierSpace(), it());
            auto u21 = u12;
            auto u22 = GetValue::get(model.ETCtensor(2_c,2_c).inFourierSpace(), it());
            auto u23 = GetValue::get(model.ETCtensor(2_c,3_c).inFourierSpace(), it());
            auto u31 = u13;
            auto u32 = u23;
            auto u33 = GetValue::get(model.ETCtensor(3_c,3_c).inFourierSpace(), it());
        ...
        template<class Model, class Looper>
        auto projectedETC_real(Model& model, Looper& it) {
            ...
            auto u11 = GetEval::getEval(model.ETCtensor(1_c,1_c).inFourierSpace(), it());
            auto u12 = GetEval::getEval(model.ETCtensor(1_c,2_c).inFourierSpace(), it());
            auto u13 = GetEval::getEval(model.ETCtensor(1_c,3_c).inFourierSpace(), it());
            auto u21 = u12;
            auto u22 = GetEval::getEval(model.ETCtensor(2_c,2_c).inFourierSpace(), it());
            auto u23 = GetEval::getEval(model.ETCtensor(2_c,3_c).inFourierSpace(), it());
            auto u31 = u13;
            auto u32 = u23;
            auto u33 = GetEval::getEval(model.ETCtensor(3_c,3_c).inFourierSpace(), it());
        ...
        auto getProjectionatpoint(Model& model, Looper& it) {
            if(mType == 0) return projectedETC_real(model, it);
            else return projectedETC_complex(model, it);
            return 0.;
        }
...
#ifdef TEMPLATTEST
#include "CosmoInterface/definitions/ETCProjector_test.h"
#endif
