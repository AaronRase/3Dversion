...
#include "CosmoInterface/measurements/measurementsIO/spectrumETCsaver.h"
...
namespace TempLat {
        ...
        PSType(par.powerSpectrumType)
 		{
        	...
        	spectraOut.emplace_back(
                			SpectrumETCSaver<T>(filesManager, "etc", amIRoot, append, par, !model.ETC)
                                    ); // File for spectra
    ...
    private:
        ...
        TempLatVector<SpectrumETCSaver<T>> spectraOut;
