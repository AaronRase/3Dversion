#ifndef COSMOINTERFACE_MEASUREMENTS_STD_SPECTRUMETCSAVERSTD_H
#define COSMOINTERFACE_MEASUREMENTS_STD_SPECTRUMETCSAVERSTD_H
...
namespace TempLat {
    ...
    template <typename T>
    class SpectrumETCSaverStd {
    public:


        SpectrumETCSaverStd(FilesManager& fm, std::string fn, bool amIRoot, bool append, const RunParameters<T>& rPar) :
        ...
        virtual ~SpectrumETCSaverStd(){}

        SpectrumETCSaverStd(FilesManager& fm, const Field<T>& fld,bool amIRoot, bool append, const RunParameters<T>& rPar) :
        mMode(!append ?  std::ios_base::out : std::ios_base::app ),
        verbosity(rPar.spectraVerbosity),
        nBins(rPar.nBinsSpectra),
        deltaKBin(rPar.deltaKBin),
    ...
    struct SpectrumETCSaverStdTester{
#ifdef TEMPLATTEST
...
#ifdef TEMPLATTEST
#include "CosmoInterface/measurements/measurementsIO/std/spectrumETCsaverstd_test.h"
#endif
