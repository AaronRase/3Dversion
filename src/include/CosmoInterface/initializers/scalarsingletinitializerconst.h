#ifndef COSMOINTERFACE_INITIALIZERS_SCALARINITIALIZERCONST_H
#define COSMOINTERFACE_INITIALIZERS_SCALARINITIALIZERCONST_H
...
namespace TempLat {
    ...
    class ScalarSingletInitializerConst {
    public:
        /* Put public methods here. These should change very little over time. */
        ScalarSingletInitializerConst() = delete;

        template<class Model, typename T>
        static void initializeScalars(Model& model, const FluctuationsGeneratorConst<T>& fg, T kCutOff, T Pconst)
        {

            // We set fluctuations to the scalar singlets:
            ForLoop(i,0,Model::Ns-1,
            	fg.conjugateGaussianFluctuations(model, model.fldS(i), model.piS(i), model.masses2S[i], model.aDotI, kCutOff, Pconst);
            );
...
#ifdef TEMPLATTEST
#include "CosmoInterface/initializers/scalarsingletinitializerconst_test.h"
#endif
