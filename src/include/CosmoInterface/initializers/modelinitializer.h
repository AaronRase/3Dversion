...
namespace TempLat {
    ...
    public:
        ...
        fgconst(model, pLSide, pSeed) //our own contribution
        {
        }
        ...
        void initialize(Model& model, RunParameters<T>& rPar)
        {
            ...
            if(Model::Ns > 0 && rPar.Pbool) ScalarSingletInitializerConst::initializeScalars(model, fgconst, rPar.kCutoff, rPar.Pconst); //our own contribution

            if(Model::Ns > 0 && !rPar.Pbool) ScalarSingletInitializer::initializeScalars(model, fg, rPar.kCutoff); //our own contribution
    ...
    private:
        ...
        FluctuationsGeneratorConst<T> fgconst; //our own contribution
    };
